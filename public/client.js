...
$(function() {
  ...
  $.get('/search-track', function(data) {
    ...
    // Display the track artist name
    
    var artistName = $('<h4>' + data.artists[0].name + '</h4>');
  ...
  $.get('/artist', function(data) {
    ...
    var followerCount = $('<h3>' + 'Followers : ' + data.followers.total + '</h3>');
    followerCount.appendTo('#artist-container');
    
     // Display the artist popularity
    var popularity = $('<h3> Popularity : ' +data.popularity + '</h3>');
